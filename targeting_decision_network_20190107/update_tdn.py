...
class TDN:
    ...
    def struct_blank_tdn(self, method, stype):
        ...
        blank_tdn = {
            ...
            'config': {'gender': {'1': [0, 0, 0, 0, 0, 0, 0, 0], '2': [0, 0, 0, 0, 0, 0, 0, 0]},
                       ...
                       'ios_version': ios_dct, 'android_version': andr_dct, 'video': tdn_mysql.select_videoid()
                       },
    ...
    def get_other_infos(self, ads, tw, tmp, master):
        ...
        if 'pt' in ads:
            if 'creative' in ads['pt'] and 'object_story_spec' in ads['pt']['creative'] and \
                    'video_data' in ads['pt']['creative']['object_story_spec'] and \
                    'video_id' in ads['pt']['creative']['object_story_spec']['video_data']:
                videoid = ads['pt']['creative']['object_story_spec']['video_data']['video_id']
                videoid = str(videoid)
                if videoid in self.res_evas[tmp]['config']['video']:
                    if ads['spend'] >= 1:
                        self.res_evas[tmp]['config']['video'][videoid][2*tw] += master
                    if ads['pay'] > 0:
                        self.res_evas[tmp]['config']['video'][videoid][2*tw+1] += master
